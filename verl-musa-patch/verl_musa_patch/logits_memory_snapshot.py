...
def _enable_allocator_history() -> None:
    import torch
...
def _install_logits_processor_snapshot() -> None:
    import torch
...
def install() -> None:
    """Install the opt-in MUSA logits memory-snapshot hooks."""
    if os.getenv("VERL_MUSA_LOGITS_SNAPSHOT", "0") != "1":
        return

    _install_phase_markers()
    _enable_allocator_history()
    _install_logits_processor_snapshot()
