...
export VERL_SGLANG_FA3_DEBUG="${VERL_SGLANG_FA3_DEBUG:-1}"
# MUSA keeps producer-side IPC storages in its limbo list after the SGLang
# consumers close them.  Collect after every completed weight bucket so those
# storages do not accumulate across updates.
export VERL_MUSA_WEIGHT_SYNC_IPC_COLLECT="${VERL_MUSA_WEIGHT_SYNC_IPC_COLLECT:-1}"
########################### Quick Config ###########################
...
HF_MODEL_PATH=${HF_MODEL_PATH:-"/ipfs/kechun.wu/models/qwen35/Qwen3___5-9B"}
...
envsubst '${VERL_PATH} ${VERL_PLUGIN_PATH} ${VERL_MUSA_PATCH} ${MEGATRON_PATH} ${MEGATRON_BRIDGE_PATH} ${MUSA_PATCH_PATH} ${VERL_MUSA_WEIGHT_SYNC_IPC_COLLECT} ${VERL_SGLANG_FA3_DEBUG}' \
