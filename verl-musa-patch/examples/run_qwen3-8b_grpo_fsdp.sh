#!/usr/bin/env bash
# GRPO | Qwen3-8B | SGLang rollout | FSDP training | Moore Threads MUSA
...
export TENSORBOARD_DIR="${TENSORBOARD_DIR:-/mnt/his_test/kechun.wu/tensorboard_logs/qwen3_8b_fsdp}"

MODEL_PATH="${MODEL_PATH:-/ipfs/models/Qwen3-8B}"
...
PROJECT_NAME="${PROJECT_NAME:-musa-ci-qwen3-8b}"
EXPERIMENT_NAME="${EXPERIMENT_NAME:-Qwen3-8B_fsdp_sglang-grpo}"
...
JOB_ID="${JOB_ID:-qwen3_8b_grpo_fsdp_musa_$(date +%Y%m%d_%H%M%S)}"
cd "${VERL_PATH}"
